...
通过将本服务挂载到您的 AI 助手上，AI 将获得以下 6 个标准工具（Tools），从而可以直接访问您的语雀进行操作：
...
- **`list_yuque_docs`**: 获取特定知识库 (book_id) 下的全部文档列表。支持传入标题关键词进行模糊搜索，直接给到 AI 具体的文档 URL Slug 标识符。
  - *输入参数*: `book_id` (知识库ID), `title_query` (可选：文章标题关键字)
